...
_cam camPreparePos [(_first_point select 0),(_first_point select 1),((_first_point select 2) + 200)];
...
_cam camPrepareTarget player;
_cam camPreparePos [((getPos player) select 0),((getPos player) select 1),(((getPos player) select 2) + 5)];
_cam camPrepareFov 0.1;
_cam camCommitPrepared _segment_time;
